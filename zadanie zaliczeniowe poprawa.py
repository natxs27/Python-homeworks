...
class MediaDatabase:
    ...
    def remove_media(self, title):
        self.data = self.data[~self.data['Title'].str.contains(title, case=False)]
        self.save_database()
        print(f"Removed {title} from the database.")

    def search_media(self, title):
        result = self.data[self.data['Title'].str.contains(title, case=False)]
        if not result.empty:
    ...
    def filter_media(self, column, value):
        if column in self.columns:
            filtered_data = self.data[self.data[column].str.contains(value, case=False)]
